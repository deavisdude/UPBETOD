...
for(var object : Collider in Physics.OverlapSphere(transform.position, SOI)){
	...
	if(object.GetComponent.<Rigidbody>() == null || object == gameObject.GetComponent.<Collider>())	continue;
	object.GetComponent.<Rigidbody>().AddForce(Physics.gravity.magnitude * GetComponent.<Rigidbody>().mass / (object.transform.position - transform.position).sqrMagnitude * (transform.position - object.transform.position).normalized, ForceMode.Acceleration);
}
}
